  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || String(t('profile.genericError')));
      }
    } catch (err) {
      setError(String(t('profile.genericError')));
    ...
    return (
      <Container maxWidth="sm">
        <Paper elevation={3} sx={{ p: 4, mt: 8, mb: 4, textAlign: 'center' }}>
          <Typography variant="h5">{String(t('profile.notLoggedIn'))}</Typography>
        </Paper>
      </Container>
    );
  }
        ...
        <Grid container spacing={4}>
          <Grid item xs={12} md={4}>
            ...
            <List>
              <ListItem>
                <ListItemText 
                  primary={String(t('profile.memberSince'))} 
              ...
              <ListItem>
                <ListItemText 
                  primary={String(t('profile.lastLogin'))} 
                  secondary={new Date().toLocaleDateString()} 
                />
              </ListItem>
            </List>
          ...
          <Grid item xs={12} md={8}>
            <Box component="form" onSubmit={handleSubmit} noValidate>
              ...
              <Typography variant="h6" sx={{ mt: 3, mb: 1 }}>
                {String(t('profile.preferences'))}
              </Typography>
              <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
                {String(t('profile.preferencesHelp'))}
              </Typography>
              
              <Box sx={{ display: 'flex', mb: 2 }}>
                <TextField
                  fullWidth
                  label={String(t('profile.newPreference'))}
                  value={newPreference}
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 3 }}>
                ...
                {preferences.length === 0 && (
                  <Typography variant="body2" color="text.secondary">
                    {String(t('profile.noPreferences'))}
                  </Typography>
                )}
              </Box>
              ...
              <Button
                ...
                disabled={loading}
              >
                {loading ? String(t('profile.saving')) : String(t('profile.saveChanges'))}
              </Button>
            </Box>
          </Grid>
        </Grid>
